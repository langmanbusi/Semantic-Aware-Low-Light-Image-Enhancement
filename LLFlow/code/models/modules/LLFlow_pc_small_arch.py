"""
LLFlow的网络结构，包含ConEncoder1和FlowUpsamplerNet
normalflow代表input输入到ConEncoder1，gt输入到FlowUpsamplerNet，获得NLLloss
reverseflow代表顺序传递数据，获得l1loss，推断时也用此方式
"""
...
from models.modules.PSA import BasicBlock
from models.modules.Fuse_Block import MultiEmbedding, TransformerBlock, TransformerBlock_1


class LLFlow_pc_small(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4, K=None, opt=None, step=None):
        super(LLFlow_pc_small, self).__init__()
        ...
        if nf == 32:
            self.att_block_1 = TransformerBlock(48, 64)
            self.att_block_2 = TransformerBlock(96, 64)
            self.att_block_3 = TransformerBlock(192, 64)
            # self.att_block_1 = TransformerBlock(64, 64)
            # self.att_block_2 = TransformerBlock(64, 64)
            # self.att_block_3 = TransformerBlock(64, 64)
        else:
            # self.att_block_1 = TransformerBlock(48, 320)
            # self.att_block_2 = TransformerBlock(96, 320)
            # self.att_block_3 = TransformerBlock(192, 320)
            self.att_block_1 = TransformerBlock_1(48, 320, 192)
            self.att_block_2 = TransformerBlock_1(96, 320, 192)
            self.att_block_3 = TransformerBlock_1(192, 320, 192)
    ...
    def forward(self, gt=None, lr=None, z=None, eps_std=None, reverse=False, epses=None, reverse_with_grad=False,
        ...
        if not reverse:
            ...
            return self.normal_flow(gt, lr, epses=epses, lr_enc=lr_enc, add_gt_noise=add_gt_noise, step=step,
                                    ...
                                    seg_map=seg_map, seg_fea=seg_fea)
        else:
            # assert lr.shape[0] == 1
            ...
            if reverse_with_grad:
                return self.reverse_flow(lr, z, y_onehot=y_label, eps_std=eps_std, epses=epses, lr_enc=lr_enc,
                                         add_gt_noise=add_gt_noise, seg_map=seg_map, seg_fea=seg_fea)
            else:
                with torch.no_grad():
                    return self.reverse_flow(lr, z, y_onehot=y_label, eps_std=eps_std, epses=epses, lr_enc=lr_enc,
                                             add_gt_noise=add_gt_noise, seg_map=seg_map, seg_fea=seg_fea)
    ...
    def normal_flow(self, gt, lr, y_onehot=None, epses=None, lr_enc=None, add_gt_noise=True, step=None,
                    align_condition_feature=False, seg_map=None, seg_fea=None):
        if self.opt['to_yuv']:
            gt = self.rgb2yuv(gt)
        if lr_enc is None and self.RRDB:
            lr_enc = self.rrdbPreprocessing(lr, seg_map, seg_fea)
        # lr_enc: [16, 32, 80, 80], fea_down4: [16, 32, 40, 40], fea_down2: [16, 32, 20, 20]
        ...
        if 'avg_color_map' in self.opt.keys() and self.opt['avg_color_map']:
            ...
            if self.RRDB is not None:
                ...
                # gt/(gt.sum(dim=1, keepdims=True) + 1e-4), 8)    # 随机选择gt或者Conditional Encoder输出的colormap作为输出
                mean = lr_enc['color_map'] if random.random() > self.opt['train_gt_ratio'] else \
                    gt/(gt.sum(dim=1, keepdims=True) + 1e-4)
    ...
    def rrdbPreprocessing(self, lr, seg_map, seg_fea):
        rrdbResults = self.RRDB(lr, seg_map, seg_fea, get_steps=True)
        ...
        if len(block_idxs) > 0:
            ...
            if opt_get(self.opt, ['network_G', 'flow', 'stackRRDB', 'concat']) or False:
                ...
                for k in keys:
                    ...
                    concat_1 = torch.cat([rrdbResults[k], F.interpolate(concat, (h, w))], dim=1)
                    if k == 'fea_up0':
                        rrdbResults[k] = self.att_block_1(concat_1, seg_fea[0])
                    elif k == 'fea_up1':
                        rrdbResults[k] = self.att_block_2(concat_1, seg_fea[1])
                    elif k == 'fea_up2':
                        rrdbResults[k] = self.att_block_3(concat_1, seg_fea[2])
                    # if k == 'fea_up0':
                    #     rrdbResults[k] = self.att_block_1(concat_1, concat_1)
                    # elif k == 'fea_up1':
                    #     rrdbResults[k] = self.att_block_2(concat_1, concat_1)
                    # elif k == 'fea_up2':
                    #     rrdbResults[k] = self.att_block_3(concat_1, concat_1)
                    else:
                        rrdbResults[k] = concat_1
        return rrdbResults
