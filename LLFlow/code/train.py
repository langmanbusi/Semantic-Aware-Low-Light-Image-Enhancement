...
def main():
    ...
    parser.add_argument('--opt', type=str, help='Path to option YMAL file.',
                            default= './confs/LOL-pc.yml') #  './confs/LOLv2-pc_rebuttal.yml') #
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            #### training
            LQ = train_data['LQ'].cuda()
            GT = train_data['GT'].cuda()
            if seg_model is not None:
                seg_map, seg_feature = seg_model(LQ[:, 0:3, :, :].cuda())
            else:
                seg_map, seg_feature = None, None
            model.feed_data_1(LQ, GT, seg_map, seg_feature)
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0:
                ...
                for val_data in val_loader:
                    ...
                    if seg_model is not None:
                        ...
                        seg_map, seg_feature = None, None
                    model.feed_data_1(LQ, GT, seg_map, seg_feature)
                    nll = model.test()   # 推理出self.fake_H
                    if nll is None:
                        nll = 0
                    nlls.append(nll)

                    visuals = model.get_current_visuals()   # 输出转移到cpu上的fake_H和GT
