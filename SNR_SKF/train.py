...
def main():
    ...
    parser.add_argument('-opt', type=str, help='Path to option YAML file.', default='./options/train/LOLv2_seg.yml')
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            #### training
            LQ = train_data['LQs'].cuda()
            nf = train_data['nf'].cuda()
            GT = train_data['GT'].cuda()
            if seg_model is not None:
                # a = time.time()
                seg_map, seg_feature = seg_model(GT)
                # b = time.time()
                # print(b-a)
            else:
            ...
            if opt['datasets'].get('val', None) and current_step % opt['train']['val_freq'] == 0:
                ...
                # if True and rank <= 0:  # image restoration validation
                    ...
                    for val_data in val_loader:
                        ...
                        nf = train_data['nf'].cuda()
                        GT = val_data['GT'].cuda()
                        LQ = util.padtensor(LQ)
                        GT = util.padtensor(GT)
                        if seg_model is not None:
                            seg_map, seg_feature = seg_model(GT)
                        else:
                            seg_map, seg_feature = None, None
                    ...
                    if opt['dist']:
                        ...
                        for idx in range(rank, len(val_set), world_size):
                            ...
                            # print(psnr_rlt[folder][idx_d])
                        # # collect data
                        # logger.info(psnr_rlt)
                        ...
                        if rank == 0:
                            ...
                            for k, v in psnr_rlt.items():
                                ...
                                psnr_rlt_avg[k] = v[0].cpu().item()
                                psnr_total_avg += psnr_rlt_avg[k]
                            # logger.info(psnr_rlt_avg)
                            # logger.info(psnr_total_avg)
                            psnr_total_avg /= len(psnr_rlt)
                            log_s = '# Validation # PSNR: {:.4e}:----Best iter: {:8,d} Best PSNR: {:.4e}'.format(psnr_total_avg, best_iter, best_psnr)
                        ...
                        log_s = '# Validation # PSNR: {:.4e}:----Best iter: {:8,d} Best PSNR: {:.4e}'.format(
                            psnr_total_avg, best_iter, best_psnr)
