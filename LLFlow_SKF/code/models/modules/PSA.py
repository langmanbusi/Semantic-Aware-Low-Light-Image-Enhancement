...
class PSA_Seg(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=1, stride=1):
        super(PSA_Seg, self).__init__()
    ...
    def spatial_pool(self, x, y):
        ...
        # [N, 1, H, W]
        context_mask = self.conv_q_right(y)
    ...
    def channel_pool(self, x, y):
        # [N, IC, H, W]
        g_x = self.conv_q_left(y)
    ...
    def forward(self, x, y):
        # [N, C, H, W]
        out = self.spatial_pool(x, y)

        # [N, C, H, W]
        out = self.channel_pool(out, y)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv_resize = conv3x3(inplanes, planes, stride)
        self.bn1 = nn.BatchNorm2d(planes, momentum=0.1)
        self.relu = nn.ReLU(inplace=True)
        self.deattn = PSA_Seg(planes, planes)
    ...
    def forward(self, x, y):
        ...
        y = self.conv_resize(y)
        out = self.conv1(x)
        out = self.bn1(out)
        out = self.relu(out)
        out = self.deattn(out, y)
        out = self.conv2(out)
        out = self.bn2(out)
